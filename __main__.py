...
def main() -> None:
    """Запускает демонстрационный сценарий загрузки задач из разных источников.

    Функция последовательно обращается к генератору, JSON-файлу и
    API-заглушке, после чего печатает полученные задачи в консоль.
    Это удобно для ручной проверки поведения приложения без запуска тестов.
    """
